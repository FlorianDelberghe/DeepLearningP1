...
class MLP(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.fc1 = nn.Linear(20, kwargs['hidden'][0])
        self.fc2 = nn.Linear(kwargs['hidden'][0], kwargs['hidden'][1])
        self.fc3 = nn.Linear(kwargs['hidden'][1], 2)
        
        self.naive = kwargs['naive']
        ...
        if kwargs['norm'] == 'batch':
            self.norm1 = nn.BatchNorm1d(20)
            self.norm2 = nn.BatchNorm1d(kwargs['hidden'][0])
            self.norm3 = nn.BatchNorm1d(kwargs['hidden'][1])
        elif kwargs['norm'] == 'dropout':
            self.norm1 = nn.Dropout(kwargs['drop_proba'][0])
            self.norm2 = nn.Dropout(kwargs['drop_proba'][1])
            self.norm3 = nn.Dropout(kwargs['drop_proba'][2])
        else :
    ...
    def forward(self, input_):
        ...
        if self.naive:
            x = torch.stack([input_[:,:10].data.max(1)[1].float(), input_[:,10:].data.max(1)[1].float()], 1)
            x = self.fcnaive(x)
        else:
            x = input_
...
def main():
    cnet2 = {"net": 'compNet2', "hidden": 120, "epochs": 80, "batch_size": 10, 
             "pool": 'max', "activation": 'relu', "naive": True, "drop_proba": [0.05, 0.05, 0.5, 0.2], "seed": None}
    cnet4 = {"net": 'compNet4', "hidden": 350, "epochs": 30, "batch_size": 10, 
             "pool": 'max', "activation": 'tanh', "naive": True, "drop_proba": [0, 0, 0, 0.2, 0, 0, 0], "seed": None}
...
#    pickle_output(cnet4)
    ...
    mlp = {"net": 'MLP', "hidden": [60, 90], "epochs": 70, "batch_size": 100, 
           "activation": 'relu', "norm": 'batch', "naive": True, "drop_proba": [0., 0., 0.6], "naive": False,  "seed": None}    
